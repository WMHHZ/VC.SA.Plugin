...
namespace utf8
{
    ...
    class invalid_code_point : public exception {
        std::uint32_t cp;
    public:
        invalid_code_point(std::uint32_t cp) : cp(cp) {}
    ...
    template <typename octet_iterator>
    std::uint32_t previous(octet_iterator& it, octet_iterator pass_start)
    {
        octet_iterator end = it;
        while (utf8::internal::is_trail(*(--it)))
            if (it == pass_start)
    ...
    // The iterator class
    template <typename octet_iterator>
    class iterator : public std::iterator <std::bidirectional_iterator_tag, std::uint32_t> {
      octet_iterator it;
